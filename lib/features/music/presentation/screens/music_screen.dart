...
/// The on-device audio library.
///
/// Streams the persisted audio rows (populated by [MediaScanService]) and
/// plays them through the shared engine via [PlayerScreen].
...
class _MusicScreenState extends State<MusicScreen> {
  ...
  Future<void> _rescan() async {
    ...
    final found = result.audioFound;
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text(
          found > 0
              ? 'Found $found track${found == 1 ? '' : 's'}'
              : 'No new audio found',
        ),
      ),
    );
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: StreamBuilder<List<MediaItem>>(
        stream: AppScope.of(context).media.watchByType(HikmahMediaType.audio),
        builder: (context, snapshot) {
          ...
          if (items.isEmpty) {
            return _StatusMessage(
              ...
              title: 'No music yet',
              message: 'Audio tracks on your device will appear here after '
          ...
          return ListView.builder(
            ...
            itemBuilder: (context, index) =>
                _TrackTile(row: items[index], theme: theme, onTap: _play),
          );
        },
      ),
    );
...
class _TrackTile extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final title = (row.title != null && row.title!.isNotEmpty)
        ? row.title!
        : row.fileName;
    final artistAlbum = [
      if (row.artist != null && row.artist!.isNotEmpty) row.artist!,
      if (row.album != null && row.album!.isNotEmpty) row.album!,
    ].join(' — ');
    final duration =
